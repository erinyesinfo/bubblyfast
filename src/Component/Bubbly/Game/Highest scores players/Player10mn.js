...
class Player30Sec extends Component {
    ...
    callAPI = async () => {
        const api = await Server.get("/readPlayer-10mn");
        if (api.data === "failure") {
            return this.setState({ errors: true });
        } else {            
            this.props.handlePlayers10Mn(JSON.parse(api.data || "[]"));
            return this.setState({ errors: false });
        }
    };
    render() {
        if (this.props.Players10Mn.length > 0) {
            return (
                <div className="players">
                    {this.props.Players10Mn.map((palyer, i) => {
                        if (i === 0 || i === 1 || i === 2) {
                            return (
...
const mapStateToProps = getState => {
    return { Players10Mn: getState.Players10Mn };
};
